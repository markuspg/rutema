...
module Rutema
  ...
  module Reporters
    ...
    ##
    # A Reporters::BlockReporter receives its data once at the end of a _rutema_
    #  run
    class BlockReporter < BaseReporter
      ##
      # Initialize a new Reporters::BlockReporter instance
      #
    ...
    ##
    # A Reporters::EventReporter receives its data continually during a _rutema_
    # run
    class EventReporter < BaseReporter
      ##
      # Initialize a new Reporters::EventReporter instance
      #
      # The +configuration+ argument is stored internally for later usage.
      ...
      # all data received through the dispatcher which was given on
      # initialization
      def run!
      ...
      ##
      # Update the Reporters::EventReporter with some data
      #
      # In the currently existing implementations the derived reporters expect
      # class instances derived from Message as data.
      def update(data) end
    end

    ##
    # This reporter is always instantiated (within Dispatcher within Engine) and
    # collects all messages fired by the _rutema_ engine
    #
